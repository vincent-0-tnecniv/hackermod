...
public class HackerHandEntityAttributeScreen extends Screen {
    ...
    public HackerHandEntityAttributeScreen(Entity entity, World world) {
...
//        }
        ...
        this.addDrawableChild(ButtonWidget.builder(
                ...
                button -> {
                    ...
                    // This differs from "changing the entity" - it requires re-summoning of the new entity
                    if(hasEntityIdChanges()){
                        if(!newEntityId.contains(":")){
                            this.newEntityId = "minecraft:" + newEntityId;
                        }
                        entityEdited.discard();
                        EntityType<?> newEntityType = Registries.ENTITY_TYPE.get(Identifier.of(newEntityId));
                        Entity newEntity = newEntityType.create(world);
                        if(newEntity != null){
                            newEntity.setPos(nx, ny, nz);
                            //// Using nx... over entityEdited.getX()... makes it so that edited values are taken into account
